 ...
 *
 * Data source that reads AnnData zarr directories directly in the browser.
 * Provides the same interface as other data sources (local-demo, local-user, remote, h5ad).
 *
 * Features:
 * - Direct zarr directory reading without server
...
import { ZarrLoader, isZarrDirectory, createZarrLoader } from './zarr-loader.js';
...
/**
 * Check if a URL uses zarr:// protocol
 * @param {string} url
 * @returns {boolean}
 */
...
/**
 * Adapter that makes ZarrLoader compatible with Cellucid data loaders
 * Caches computed data and provides efficient access patterns
 */
class ZarrDataAdapter {
  /**
  ...
  async _computeObsFieldsMetadata() {
    ...
    for (const key of this._loader.obsKeys) {
      try {
        ...
        if (fieldInfo.dtype === 'categorical') {
        ...
        } else if (fieldInfo.dtype === 'string') {
          // String treated as categorical - need to read values to get unique set
          ...
          // Unknown type - skip
          console.warn(`[ZarrDataAdapter] Skipping obs field '${key}' with unknown dtype: ${fieldInfo.dtype}`);
        }
      } catch (err) {
        console.warn(`[ZarrDataAdapter] Failed to process obs field '${key}':`, err);
      }
    }
  ...
  async _getOrLoadFieldData(key) {
    ...
    if (meta.kind === 'continuous') {
      ...
      // Handle both TypedArray and Array
  ...
  getObsManifest() {
    ...
    for (const field of this._obsFieldsMetadata) {
      if (field.kind === 'continuous') {
        ...
        // Note: Browser zarr loader returns empty centroids for performance.
        const centroids = {};
...
export class ZarrDataSource {
  ...
  async loadFromFileList(fileList) {
    ...
    this.datasetId = `zarr_${this.dirname.replace('.zarr', '')}`;
