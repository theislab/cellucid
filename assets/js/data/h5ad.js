...
export class H5adLoader {
  ...
  async getDatasetMetadata() {
    ...
    // so one file opens the same way here and through `cellucid serve`.
    if (availableDimensions.size === 0 && this._obsmKeys.includes(UNSUFFIXED_EMBEDDING_KEY)) {
      const { nDims } = this.getEmbeddingShape(UNSUFFIXED_EMBEDDING_KEY);
    ...
    if (availableDimensionsList.length === 0) {
      throw new Error(
        'No UMAP embedding this viewer can read was found in obsm. Expected one ' +
        'or more of X_umap_1d, X_umap_2d, or X_umap_3d, or a plain X_umap of 1, ' +
        '2, or 3 columns. ' +
